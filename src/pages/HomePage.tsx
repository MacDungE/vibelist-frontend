  ...
  return (
    <div className="min-h-screen w-full font-sans" style={{ background: 'var(--bg)', color: 'var(--text-primary)' }}>
      <div className="flex flex-col w-full max-w-[600px] mx-auto px-0 min-h-screen">
        {/* 메인 컨텐츠 */}
        <main
          className="w-full flex-1 flex flex-col items-center justify-center h-full min-h-0 overflow-y-auto"
          id="main-content"
        >
          {/* Step 1 - 감정 선택 */}
          <StepCard
            ...
            data-step="1"
          >
            <div className="text-center scale-[0.85] sm:scale-100 transform-gpu">
              <h2 className="text-lg text-gray-700 mb-2 sm:mb-3">
                지금 당신의 기분은 어떤가요?
              </h2>
              <p className="text-2xl mb-2 sm:mb-4 text-gray-700">
            ...
            <div className="flex justify-center mt-8">
              <button
                ...
                className="!rounded-button whitespace-nowrap bg-gradient-to-r from-[#4A6CF7] to-[#9B8CFF] text-[#F7F9FC] px-10 py-4 rounded-full shadow-md hover:shadow-lg hover:translate-y-[-3px] transition-all duration-300 cursor-pointer font-medium flex items-center gap-2 group relative overflow-hidden"
              >
                <span className="relative z-10">다음</span>
                <i className="fas fa-arrow-right relative z-10"></i>
                <div className="absolute inset-0 bg-white/20 -translate-x-full skew-x-12 group-hover:animate-shine"></div>
              </button>
            </div>
          </StepCard>
          ...
          {(activeStep >= 2 || activeStep === 1) && (
            ...
            >
              ...
              <MoodChangeSelector
                ...
                selectedEmotion={getEmotionState(position.x, position.y)}
              />
              <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 w-full">
                <button
                  onClick={() => goToStep(1)}
                  className="!rounded-button whitespace-nowrap px-8 py-4 text-gray-600 hover:text-indigo-600 font-medium flex items-center gap-2 cursor-pointer transition-all duration-300 hover:translate-y-[-3px]"
                >
                ...
                <button
                  ...
                  className={`!rounded-button whitespace-nowrap bg-gradient-to-r from-[#4A6CF7] to-[#9B8CFF] text-[#F7F9FC] px-10 py-4 rounded-full shadow-md hover:shadow-lg hover:translate-y-[-3px] transition-all duration-300 cursor-pointer font-medium flex items-center gap-2 group relative overflow-hidden ${!selectedMoodChange ? "cursor-not-allowed opacity-50 hover:translate-y-0 hover:shadow-none" : ""}`}
                >
                  <span className="relative z-10">플레이리스트 만들기</span>
                  <i className="fas fa-music ml-1 relative z-10"></i>
                  <div
                    className={`absolute inset-0 bg-white/10 -translate-x-full skew-x-12 ${selectedMoodChange ? "group-hover:animate-shine" : ""}`}
                  ></div>
                </button>
              </div>
            </StepCard>
